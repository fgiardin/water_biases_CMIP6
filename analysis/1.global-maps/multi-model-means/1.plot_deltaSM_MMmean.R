...
# assign same color to all outliers (to ensure that the structure of the data is highlighted properly)
# only affects GRACE data (thus won't affect calculation of multi-model mean!!)
...
summary_deltaSM_MMmeans <- summary_deltaSM %>%
  bind_rows(MMmeans)
...
# save dataframe
saveRDS(summary_merged, "summary_deltaSM_MMmean.rds", compress = "xz")

# create the list of models to print
#model_list <- unique(summary_merged$model_name)
...
plot_list <- lapply(model_list, function(model) {
  ...
  if(model != "GRACE observations") {
    ...
    mean_bias_abs <- df_model %>%
      mutate(mean_bias_abs = abs(deltaSMmax - deltaSMmax_GRACE)) %>%
      pull(mean_bias_abs) %>%
      mean(na.rm = TRUE)
    ...
    # calculate mean bias
    mean_bias <- df_model %>%
      mutate(mean_bias = deltaSMmax - deltaSMmax_GRACE) %>%
      pull(mean_bias) %>%
      mean(na.rm = TRUE)
  ...
  p <- ggplot() +
    ...
    guides(fill = guide_colourbar(frame.linewidth = 0.5,
           ...
           color = guide_colourbar(frame.linewidth = 0.5,
                                   ...
                                   frame.colour = "black",
                                   ticks.colour = "black")) +
    labs(title = model, # fill and color same label --> only one colorbar in the legend
...
saveRDS(plot_list, "plot_list_SMmax.rds", compress = "xz")
