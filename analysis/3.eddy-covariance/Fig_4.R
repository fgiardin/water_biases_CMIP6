...
# flux
plot_list[['site1_flux']] <- plot_flux(df_daily_flux, site1)
plot_list[['site2_flux']] <- plot_flux(df_daily_flux, site2)
plot_list[['site3_flux']] <- plot_flux(df_daily_flux, site3)
...
# Loop over models
  for (pmodel in plot_model) {
    ...
    plot_list[[plot_name_site1]] <- plot_flux(filtered_data, site1)

    # Generate and save plot for site2
    plot_name_site2 <- paste("site2", pmodel, sep = "_")
    plot_list[[plot_name_site2]] <- plot_flux(filtered_data, site2)

    # Generate and save plot for site3
    plot_name_site3 <- paste("site3", pmodel, sep = "_")
    plot_list[[plot_name_site3]] <- plot_flux(filtered_data, site3)
  }
  ...
  # Arrange the plots in a 5x3 grid (5 columns for the observation + 4 models, 3 rows for the sites)
  all <- ggarrange(plotlist = plots_in_order,
                   labels = "auto",
                   ncol = 5, nrow = 3) %>%
    annotate_figure(top = text_grob(c("FLUXNET2015", "UKESM1-0-LL", "IPSL-CM6A-LR", "EC-Earth3-Veg", "CNRM-ESM2-1"),
                                    x = c(0.1, 0.3, 0.5, 0.7, 0.9), # x position of each column title
                                    size = 17,
                                    face = "bold"))

  ggsave("EFvsSM_sites.png", path = "./", width = 18, height = 10.8, dpi= 300) # width = 8, height = 15,
