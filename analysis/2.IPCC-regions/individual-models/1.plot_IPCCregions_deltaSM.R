# script to plot deltaSM results grouped by IPCC region
# results by model
...
# load SM data
summary_deltaSM_allyears <- readRDS("data/deltaSM/summary_deltaSM_allyears.rds")
...
# merge to deltaSM
df_plot <- summary_deltaSM_allyears %>%
  left_join(df_IPCC, by = join_by(lon, lat)) %>%
  drop_na() %>%
  group_by(Region, model_name) %>% # in every IPCC cell and per every model, calculate median deltaSMmax
  summarise(medianSMmax = mean(deltaSMmax, na.rm = TRUE)) %>%
  ungroup()

# Filter out the data for the .GRACE model.
grace_data <- df_plot %>%
  filter(model_name == ".GRACE") %>%
...
df_merged <- full_join(df_plot %>% dplyr::filter(model_name != ".GRACE"), # remove GRACE in the original dataframe to avoid repetitions
...
for(model in unique_models) {
  ...
  fit <- lm(medianSMmax ~ medianSMmax_GRACE, data = df_model)
  r_squared <- summary(fit)$r.squared
  r_squared_label <- bquote(italic(R)^2 == .(round(r_squared, 2)))

  plots[[model]] <- ggplot(data = df_model,
                           aes(x = medianSMmax_GRACE, # same x-axis for all sub-plots
                               y = medianSMmax,
                               color = Region,
    ...
    labs(x = "GRACE (mm)", # expression(paste(Delta*SM[max], " GRACE  (mm)"))
         y = paste(model, "(mm)"),
...
all_final <- annotate_figure(all,
                             top = text_grob(
                               expression(bold(paste(Delta*SM[max], ": maximum depletion of total-column soil moisture"))),
                               size = 16
                               )
                             )
...
ggsave("IPCC_regions_deltaSM.png", path = "./", width = 11, height = 12, dpi= 600) # width = 8, height = 15,
