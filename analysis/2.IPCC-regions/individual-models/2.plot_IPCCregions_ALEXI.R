# script to plot CWD_max results in each IPCC region
# results per model
...
# load SM data
summary_cwd <- readRDS("data/CWD/max_CWD/summary_max_cwd.rds")
...
df_merged <- full_join(df_plot %>% dplyr::filter(model_name != "ALEXI observations"), # remove ALEXI in the original dataframe to avoid repetitions
...
all_final <- annotate_figure(all,
                             top = text_grob(
                               "Maximum Cumulative Water Deficit over a 80-year period",
                               size = 16,
                               face = "bold"
                             )
)

ggsave("IPCC_regions_maxCWD.png", path = "./", width = 11, height = 12, dpi= 600) # width = 8, height = 15,
