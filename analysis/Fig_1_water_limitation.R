# script to plot frequency of water limitation (multi-model mean) [Fig. 1]
# switch scenario to "historical" for figure in supplementary material
...
# *** select scenario ***
# Note: run one scenario per session to avoid mixing outputs
scenario_select = "land-hist" # options: "land-hist" or "historical"

# load data
df_count_GRACE <- readRDS("data/theta_crit/monthly/df_count_GRACE.rds") %>%
  dplyr::select(-date, -SIF, -TWS) %>%  # drop date, SIF and TWS columns
  unique() %>%
